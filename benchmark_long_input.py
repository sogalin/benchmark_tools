...
def run_benchmark(input_sizes, num_runs):  
    # List to collect all Ave_us values  
    all_ave_us_values = []  
    ...
    for INPUT_SIZE in input_sizes:  
        for run in range(num_runs):  
            ...
            # Get Ave_us column data and append it to the master list  
            ave_us_values = filtered_df['Ave_us'].tolist()  
            all_ave_us_values.extend(ave_us_values)  
  
            # Print the Ave_us values for each run  
            print(f"Ave_us values for input size {INPUT_SIZE}, run {run+1}: {ave_us_values}")  
  
    # Print all collected Ave_us values  
    print("All Ave_us values from all test cases:", all_ave_us_values)  
